...
- [ ] In site/admin-reports.html, the nav bar's `.nav-links` list currently has "توثيق الوسطاء" and "البلاغات" (self-link). Add one more item after them: "كل الطلبات" linking to admin-requests.html, same `<li><a>` style. Change nothing else in the file. Touch only site/admin-reports.html — nothing under lib/, no other file.
