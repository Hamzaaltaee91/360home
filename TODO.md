...
- [ ] Add `purpose` column to `public.property_requests`: TEXT NOT NULL CHECK (purpose IN ('rent', 'buy')), no default going forward — but existing rows must be backfilled to 'rent' first in the same migration before the NOT NULL constraint is applied.
...
- [ ] Update the property_requests schema section in CLAUDE.md to document the five new columns added by the migrations above, matching their exact names, types, and constraints.
- [ ] Create site/js/iraq-locations.js exporting one plain object mapping each of Iraq's 18 governorate slugs to { label: Arabic name, areas: [{ value, label }, ...] }, 8 to 15 common areas per governorate, every list ending with { value: "other", label: "أخرى" }.
- [ ] Create site/js/numerals.js exporting one function toWesternDigits(str) that maps Arabic-Indic digits ٠-٩ to Western digits 0-9 and leaves every other character unchanged.
- [ ] In site/request-new.html, wrap the existing fields into five fieldset groups with legends: الفئة والغرض, الموقع, الميزانية, المواصفات, تفاصيل إضافية — keep every existing field id used by the submit handler exactly as-is.
- [ ] In site/request-new.html, add a required purpose field (two options: rent/buy, Arabic labels إيجار/شراء) in the الفئة والغرض fieldset with neither option pre-selected, and block form submission until one is chosen.
- [ ] In site/request-new.html, replace the free-text "المدينة" input with a "المحافظة" select populated from site/js/iraq-locations.js, and replace "المنطقة" with a dependent select that repopulates from the chosen governorate's areas list on change, revealing a free-text fallback input when "أخرى" is chosen.
- [ ] In site/request-new.html, replace the separate "أقل سعر" and "أعلى سعر" inputs with a single "سقف الميزانية" number input; in the submit handler compute min_price as the entered value times 0.7 rounded to the nearest integer, and send both min_price and max_price to createRequest.
- [ ] In site/request-new.html, add a "نوع العقار" select (شقة/بيت/فيلا/دوبلكس) shown only when الفئة = سكني and hidden and cleared to null otherwise, wired to the property_subtype field.
- [ ] In site/request-new.html, add a "مدة الإيجار" select (يومي/أسبوعي/شهري/سنوي) defaulting to شهري, shown only when purpose = rent and hidden and cleared to null when purpose = buy.
- [ ] In site/request-new.html, attach site/js/numerals.js's toWesternDigits to the input event of every numeric field (budget, bedrooms, bathrooms) so Arabic-Indic digits convert live as the user types.
- [ ] In site/request-new.html, replace the single generic #error banner with a separate inline error message element under each field that can fail validation.
- [ ] In site/dashboard.html, add tags for المحافظة, الغرض, and سقف الميزانية to the request card rendering loop alongside the existing category/city tags, using the same createElement/textContent pattern already used in that file — do not introduce innerHTML.
- [ ] In site/request.html, add rendering for purpose, governorate, area, property_subtype, and rental_period (when not null) to the request detail card, using the same safe DOM-building pattern already used in that file.
