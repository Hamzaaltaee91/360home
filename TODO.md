...
- [x] In lib/screens/admin/admin_dashboard_screen.dart, add 7 new navigation entries below the existing 'توثيق الوسطاء'/'إدارة المستخدمين' cards (same ListTile/Card style already used there): 'القوائم المنسدلة' → context.push('/admin/lists'), 'المحافظات والمناطق' → context.push('/admin/locations'), 'البلاغات' → context.push('/admin/reports'), 'طلبات العقارات' → context.push('/admin/requests'), 'عروض الوسطاء' → context.push('/admin/offers'), 'دليل الوسطاء' → context.push('/admin/realtors-directory'), 'سجل النشاطات' → context.push('/admin/audit-log'). Keep all existing stats/refresh functionality and the two existing nav entries exactly as they are. Touch only lib/screens/admin/admin_dashboard_screen.dart — nothing under lib/routes/, no other file.
- [x] In lib/screens/buyer/create_request_screen.dart, add a currency selector: load options via SupabaseService().getListOptions('currency') in initState (list of {code, label} maps) into a new state field, default the selected currency to 'IQD' if present in the loaded list else the first option; render a DropdownButtonFormField near the existing price fields showing each option's label with its code as value; pass `currency: _selectedCurrency` (the selected code) to the existing SupabaseService().createPropertyRequest(...) call, which currently omits currency and silently gets the DB default 'AED' — wrong default for an Iraq-focused app. If the currency list hasn't loaded yet when the form is submitted, fall back to 'IQD'. Touch only lib/screens/buyer/create_request_screen.dart — nothing under lib/services/, no other file.
- [x] In lib/screens/realtor/create_offer_screen.dart, add a currency selector: load options via SupabaseService().getListOptions('currency') in initState (list of {code, label} maps) into a new state field, default the selected currency to 'IQD' if present in the loaded list else the first option; render a DropdownButtonFormField near the existing price field showing each option's label with its code as value; pass `currency: _selectedCurrency` (the selected code) to the existing SupabaseService().createOffer(...) call, which currently omits currency and silently gets the DB default 'AED' — wrong default for an Iraq-focused app. If the currency list hasn't loaded yet when the form is submitted, fall back to 'IQD'. Touch only lib/screens/realtor/create_offer_screen.dart — nothing under lib/services/, no other file.
- [x] In site/become-realtor.html and site/js/verification.js together: add a WhatsApp phone input field to the form in site/become-realtor.html (label "رقم واتساب", type tel, required, placed near the existing company-name/license-number fields, with the same error-span pattern already used for the other fields there); change submitRealtorApplication in site/js/verification.js to accept a 5th parameter whatsappPhone and pass it as p_whatsapp_phone to the submit_realtor_application RPC call (the RPC already supports this — Flutter's verification_screen.dart already requires and sends this field, but the website form never collects it, so every realtor application submitted via the website has always left this column NULL); update the caller in become-realtor.html's submit handler to read the new field's value and pass it as the 5th argument. Touch only these two files — nothing under lib/, no other file.
- [x] Security fix: in site/js/verification.js, change uploadLicenseDocument(userId, file) to upload to the private 'verification-documents' bucket instead of the public 'dabberli' bucket — path `${userId}/${file.name}`, using `supabase.storage.from("verification-documents").upload(path, file)`, and return the path string (not a public URL); remove the getPublicUrl call. Add a new exported async function getVerificationDocumentUrl(path) that calls `const { data, error } = await supabase.storage.from("verification-documents").createSignedUrl(path, 3600); if (error) throw error; return data.signedUrl;` — mirrors lib/services/supabase_service.dart's uploadVerificationDocument/getVerificationDocumentUrl (private bucket, only uploader/admin can read per storage policy). Touch only site/js/verification.js — nothing under lib/, no other file.
- [x] Security fix: in site/admin-realtors.html, replace the direct `docLink.href = app.document_url;` (which currently points at a public bucket URL) with an async call to `getVerificationDocumentUrl(app.document_url)` (import it from ./js/verification.js — add the import if not already present) to get a time-limited signed URL before setting docLink.href, since document_url is now a private-bucket storage path rather than a public URL. Change nothing else in the file. Touch only site/admin-realtors.html — nothing under lib/, no other file.
- [BLOCKED] fix: site/js/verification.js `uploadLicenseDocument` uploads realtor KYC docs to public 'dabberli' bucket and returns a public URL — switch to private 'verification-documents' bucket (path `${userId}/${file.name}`, matching lib/services/supabase_service.dart's uploadVerificationDocument), drop getPublicUrl, and add a signed-URL helper mirroring getVerificationDocumentUrl for display. Touch only site/js/verification.js — nothing under lib/, no other file.
- [x] In site/js/numerals.js, add two new exported pure functions: formatThousands(value) that takes a number or numeric string, strips anything but digits, and returns it with Western-digit thousands separators (commas) inserted every 3 digits from the right (e.g. 750000 -> '750,000'; empty/invalid input -> ''); and stripThousands(str) that removes comma characters and returns the plain digit string (e.g. '750,000' -> '750000'). Keep the existing toWesternDigits function unchanged. Touch only site/js/numerals.js — nothing under lib/, no other file.
- [x] In site/js/auth.js, add a new exported async function getCurrentUserDisplay() that calls supabase.auth.getUser(), and if there is a signed-in user, queries the users table (select full_name, email, filtered by .eq('auth_id', user.id), .single()) and returns { full_name, email }; returns null if no user is signed in. Mirror the existing getCurrentAppUserId function's error-handling style (throw on query error). Touch only site/js/auth.js — nothing under lib/, no other file.
- [x] In site/request-new.html, import formatThousands and stripThousands from ./js/numerals.js. Change the #max-price input's existing input-event handler so that, after converting Arabic-Indic digits to Western and stripping non-digits (existing sanitizeNumericInput behavior), it also re-formats the input's displayed value with formatThousands (so typing 750000 shows '750,000' live), preserving cursor position the same way the existing caret-restore logic does. When reading the field's value for submission (the num('max-price') call used to build the createRequest payload), strip the thousands separators via stripThousands before calling Number() so the value sent to createRequest is still a plain integer with no commas. Keep every other field and validation on this page exactly as it is. Touch only site/request-new.html — nothing under lib/, no other file.
- [x] In site/request-new.html, the submit button (`<button type="submit" class="btn btn-primary btn-lg">انشر الطلب</button>`) sits directly against the error-text `<p id="form-error">` above it with no visual gap. Add `style="margin-top: 24px;"` to that button element. Change nothing else in the file. Touch only site/request-new.html — nothing under lib/, no other file.
- [x] In site/dashboard.html, import getCurrentUserDisplay from ./js/auth.js. After requireSession() resolves, call it and, if it returns a non-null value, insert a new <span> showing the user's full_name (or email if full_name is empty) right before the existing #signout-btn button inside .nav-actions, so the signed-in user's name is visible in the nav. If it returns null or throws, leave the nav as it is today (don't block the page). Keep every other element and behavior on this page exactly as it is. Touch only site/dashboard.html — nothing under lib/, no other file.
- [x] In site/dashboard.html, import formatThousands from ./js/numerals.js. In the script block that renders each request card: (1) replace the raw r.category value shown in catTag.textContent with its Arabic label (residential -> 'سكني', commercial -> 'تجاري', land -> 'أرض', falling back to the raw value for anything else); (2) change budgetTag.textContent so, when r.max_price is not null, it shows formatThousands(r.max_price) followed by ' د.ع' (e.g. '750,000 د.ع'), and '—' when null. Keep every other tag (city, governorate, purpose) and all other behavior on this page exactly as it is. Touch only site/dashboard.html — nothing under lib/, no other file.
- [x] In site/index.html, add a new `<script type="module">` block right before `</body>` that imports { supabase } from './js/supabase-client.js', calls `const { data } = await supabase.auth.getSession()`, and if `data.session` exists, immediately redirects via `window.location.href = 'dashboard.html'`. Change nothing else in the file — the landing page's static HTML stays exactly as it is for signed-out visitors. Touch only site/index.html — nothing under lib/, no other file.
- [x] In site/create-offer.html, import formatThousands and stripThousands from ./js/numerals.js. Change the #offered-price input's existing input-event handler (in the numericInputs sanitizeNumericInput wiring) so that, after converting Arabic-Indic digits to Western and stripping non-digits, it also re-formats the input's displayed value with formatThousands (so typing 750000 shows '750,000' live), preserving cursor position the same way the existing caret-restore logic does. When building the createOffer payload, strip the thousands separators via stripThousands from the offered-price field's value before calling Number() so offered_price sent to createOffer is still a plain integer with no commas. Keep every other field on this form (including area-sqft/bedrooms/bathrooms sanitization) exactly as it is. Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] In site/create-offer.html, remove the 'نوع العرض' form field entirely: delete the `<div class="form-field"><label for="lease-type">...` block containing the #lease-type select, and update the createOffer(...) call in the submit handler to stop passing `lease_type: val('lease-type')` (remove that line from the payload object entirely, so lease_type is omitted). Keep every other field and validation on this form exactly as it is. Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] In site/create-offer.html, change the #bedrooms and #bathrooms fields from free-text numeric `<input type="text" inputmode="numeric">` elements to `<select>` dropdowns, each with a disabled selected placeholder option "— اختر —" followed by options '1' through '5' (matching the same value pattern already used for the #bedrooms/#bathrooms selects in site/request-new.html). Remove the 'bedrooms' and 'bathrooms' entries from the numericInputs sanitizeNumericInput wiring array (selects don't need digit sanitization). Keep the existing numOrNull() reads for these two fields unchanged (a select's value is still a string, so numOrNull continues to work). Keep every other field on this form exactly as it is. Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] In site/create-offer.html's submit handler, add required-field validation (matching the existing showFieldError/hasError pattern already used for property-title/property-address/offered-price) for: #area-sqft (must not be empty), #bedrooms (must not be empty), #bathrooms (must not be empty), and at least one file selected in #property-photos (show its error via a new `<span class="error-text" data-error-for="property-photos" hidden></span>` added right after that file input, matching the markup pattern of the other error spans). If any of these four are missing, show its field error and set hasError = true, same as the existing checks, before the form submits. Keep every other field and validation on this form exactly as it is. Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] In site/js/auth.js, add a new exported async function getRoleHomePath() that resolves the signed-in user's role (reuse the same users-table lookup pattern as getCurrentUserDisplay: query .from("users").select("role").eq("auth_id", user.id).single() after supabase.auth.getUser()) and returns 'realtor-dashboard.html' when role is 'realtor', 'admin-realtors.html' when role is 'admin', and 'dashboard.html' for 'buyer' or any other/missing value (including when nobody is signed in — never throw). Touch only site/js/auth.js — nothing under lib/, no other file.
- [x] In site/index.html, in the script block added earlier that redirects a signed-in visitor, import getRoleHomePath from ./js/auth.js and replace the hardcoded `window.location.href = "dashboard.html"` with `window.location.href = await getRoleHomePath()`. Keep the existing try/catch and session check exactly as they are. Touch only site/index.html — nothing under lib/, no other file.
- [x] In site/admin-realtors.html, the nav bar's self-referencing link currently reads "طلباتي" and points to dashboard.html (the buyer page) — wrong for an admin. Change its text to "توثيق الوسطاء" and its href to "admin-realtors.html". Change nothing else in the file. Touch only site/admin-realtors.html — nothing under lib/, no other file.
- [x] In site/admin-realtors.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#applications-list` container, visible by default; before the initial call that populates the applications list (listPendingRealtorApplications), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/admin-realtors.html — nothing under lib/, no other file.
- [x] In site/admin-realtors.html, three catch blocks currently only console.error and silently degrade the UI with no message to the admin: (1) the one that removes the document link when getVerificationDocumentUrl fails, (2) the one in the approve-application handler, (3) the one in the reject-application handler. Add a shared `<p id="page-error" role="status" hidden></p>` element near the top of the page content (after the page-head, before the applications list), and in all three catch blocks, in addition to the existing console.error, set this element's textContent to a short Arabic message describing what failed (e.g. "تعذّر تحميل المستند" / "تعذّر قبول الطلب" / "تعذّر رفض الطلب") and unhide it. Keep every other behavior on this page exactly as it is. Touch only site/admin-realtors.html — nothing under lib/, no other file.
- [x] In site/admin-reports.html, the nav bar's self-referencing link currently reads "طلباتي" and points to dashboard.html (the buyer page) — wrong for an admin. Change its text to "البلاغات" and its href to "admin-reports.html", and add a new nav link "توثيق الوسطاء" pointing to admin-realtors.html next to it, so an admin on the reports page has a way back to the verification queue. Change nothing else in the file. Touch only site/admin-reports.html — nothing under lib/, no other file.
- [x] In site/admin-reports.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#reports-list` container, visible by default; before the initial call that populates the reports list (listOpenReports), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/admin-reports.html — nothing under lib/, no other file.
- [x] In site/admin-reports.html, two catch blocks currently only console.error and silently re-enable their buttons with no message to the admin: the "تمت المراجعة" (resolveReport ... "reviewed") handler and the "رفض البلاغ" (resolveReport ... "dismissed") handler. Add a shared `<p id="page-error" role="status" hidden></p>` element near the top of the page content (after the page-head, before the reports list), and in both catch blocks, in addition to the existing console.error, set this element's textContent to a short Arabic message (e.g. "تعذّر تحديث حالة البلاغ") and unhide it. Keep every other behavior on this page exactly as it is. Touch only site/admin-reports.html — nothing under lib/, no other file.
- [x] In site/dashboard.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#requests-list` container, visible by default; before the initial call that populates it (listMyRequests), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/dashboard.html — nothing under lib/, no other file.
- [x] In site/request-new.html, the nav bar's `.nav-links` currently has only "طلباتي" (dashboard.html). Add two more items after it, matching site/dashboard.html's nav-links pattern exactly: "الإشعارات" linking to notifications.html, and "سجل كوسيط" linking to become-realtor.html. Change nothing else in the file. Touch only site/request-new.html — nothing under lib/, no other file.
- [x] In site/request.html, the nav bar's `.nav-links` currently has only "طلباتي" (dashboard.html). Add two more items after it, matching site/dashboard.html's nav-links pattern exactly: "الإشعارات" linking to notifications.html, and "سجل كوسيط" linking to become-realtor.html. Change nothing else in the file. Touch only site/request.html — nothing under lib/, no other file.
- [x] In site/request.html, add two loading indicators: a `<p id="detail-loading">جاري التحميل...</p>` immediately before the `#request-detail` container, hidden once the initial getRequest() call settles; and a separate `<p id="offers-loading">جاري التحميل...</p>` immediately before the `#offers-list` container, hidden once the initial listOffersForRequest() call settles. Both visible by default, both hidden in a try/finally (or equivalent) so they hide on success or failure. Keep every other element and behavior on this page exactly as it is. Touch only site/request.html — nothing under lib/, no other file.
- [x] In site/request.html, the delete-request failure handler currently shows a generic `window.alert("تعذّر حذف الطلب، حاول مرة أخرى.")` instead of matching the rest of the site's inline error-text pattern (a `<span class="error-text">` or `<p class="error-text">` element). Replace that alert() call with: add a `<p class="error-text" id="delete-error" hidden></p>` element near the delete button, and on failure set its textContent to the same message and unhide it instead of alerting. Keep every other behavior on this page exactly as it is. Touch only site/request.html — nothing under lib/, no other file.
- [x] In site/edit-request.html, the nav bar's `.nav-links` currently has only "طلباتي" (dashboard.html). Add two more items after it, matching site/dashboard.html's nav-links pattern exactly: "الإشعارات" linking to notifications.html, and "سجل كوسيط" linking to become-realtor.html. Change nothing else in the file. Touch only site/edit-request.html — nothing under lib/, no other file.
- [x] In site/edit-request.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before `<form id="request-form">` (the form is currently visible with blank/default fields until the initial getRequest() call resolves and populates it). Show the indicator by default, hide the form's fieldsets (or just leave the indicator visible on top — simplest: hide the indicator once getRequest() settles, in a try/finally or equivalent, without altering the form itself). Keep every other element and behavior on this page exactly as it is. Touch only site/edit-request.html — nothing under lib/, no other file.
- [x] In site/become-realtor.html, the nav bar's `.nav-links` currently has only "طلباتي" (dashboard.html). Add one more item after it, matching site/dashboard.html's nav-links pattern: "الإشعارات" linking to notifications.html. Change nothing else in the file. Touch only site/become-realtor.html — nothing under lib/, no other file.
- [x] In site/become-realtor.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element near the top of the page content (before whichever of the realtor-application form or the status view is shown), visible by default; the application form currently flashes visible even for users who already have a pending/approved application until the initial refreshStatus() call resolves. Hide the loading indicator once that initial call settles (success or failure, try/finally or equivalent). Keep every other element and behavior on this page exactly as it is. Touch only site/become-realtor.html — nothing under lib/, no other file.
- [x] In site/realtor-requests.html, the nav bar's `.nav-links` currently has only "الطلبات المتاحة" (self-link). Add two more items after it: "عروضي" linking to my-offers.html, and "الإشعارات" linking to notifications.html. Change nothing else in the file. Touch only site/realtor-requests.html — nothing under lib/, no other file.
- [x] In site/realtor-requests.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#requests-list` container, visible by default; before the initial call that populates it (listActiveRequestsForRealtor), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/realtor-requests.html — nothing under lib/, no other file.
- [x] In site/my-offers.html, the nav bar's `.nav-links` currently has only "عروضي" (self-link). Add two more items after it: "تصفح الطلبات" linking to realtor-requests.html, and "الإشعارات" linking to notifications.html. Change nothing else in the file. Touch only site/my-offers.html — nothing under lib/, no other file.
- [x] In site/my-offers.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#offers-list` container, visible by default; before the initial call that populates it (listMyOffers), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/my-offers.html — nothing under lib/, no other file.
- [x] In site/create-offer.html, the nav bar's `.nav-links` currently has only "عروضي" (my-offers.html). Add two more items after it: "تصفح الطلبات" linking to realtor-requests.html, and "الإشعارات" linking to notifications.html. Change nothing else in the file. Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] In site/notifications.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#notifications-list` container, visible by default; before the initial call that populates it (listMyNotifications), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/notifications.html — nothing under lib/, no other file.
- [x] In site/notifications.html, the "تحديد كمقروء" button's markAsRead(n.id) catch block currently only re-enables the button with no visible error. Add a `<p class="error-text" id="page-error" hidden></p>` element near the top of the notifications list, and in that catch block set its textContent to a short Arabic message (e.g. "تعذّر تحديث حالة الإشعار") and unhide it, in addition to keeping the button re-enable behavior. Keep every other behavior on this page exactly as it is. Touch only site/notifications.html — nothing under lib/, no other file.
- [x] In site/messages.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#messages-list` container, visible by default; before the initial call that populates it (listMessages(offerId)), leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/messages.html — nothing under lib/, no other file.
- [x] In site/messages.html, the onSend() function's catch block currently only does console.error(err) when sendMessage fails, leaving the user's typed message silently gone with no feedback. Reuse the existing `#safety-status` element (the same one the report-button handler already sets text on and unhides) in this catch block too: set its textContent to a short Arabic message (e.g. "تعذّر إرسال الرسالة، حاول مرة أخرى.") and unhide it, in addition to the existing console.error. Keep every other behavior on this page exactly as it is. Touch only site/messages.html — nothing under lib/, no other file.
- [x] In site/messages.html, the block that loads and renders the initial message list currently only does console.error(err) on failure, leaving the page showing an empty/broken list with no explanation. Add a `<p class="error-text" id="messages-error" hidden></p>` element near the `#messages-list` container, and in that catch block set its textContent to a short Arabic message (e.g. "تعذّر تحميل الرسائل") and unhide it, in addition to the existing console.error. Keep every other behavior on this page exactly as it is. Touch only site/messages.html — nothing under lib/, no other file.
- [x] In site/offer.html, there is currently no nav bar or sign-out button anywhere on the page (every other authenticated page has one). Add a `<nav class="site-nav">` at the top of the body matching site/request.html's nav pattern exactly: the "دبّرلي" logo link to index.html, a nav-links list with "طلباتي" linking to dashboard.html, and a nav-actions div with a `#signout-btn` button wired to signOut from ./js/auth.js (import signOut alongside whatever is already imported from that module). Keep the existing `#back-link` and all other page content exactly as it is. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/offer.html, add a loading indicator: insert a `<p id="loading-msg">جاري التحميل...</p>` element immediately before the `#offer-detail` container, visible by default; before the initial getOffer() call, leave it visible, and once that call settles (success or failure — use try/finally or equivalent so it always hides), set its `hidden` attribute to true. Keep every other element and behavior on this page exactly as it is. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/offer.html, the respond(response) function currently has no try/catch at all around `await respondToOffer(offerId, response)`, and the #interested-btn/#not-interested-btn buttons are not disabled during that await — a failure is completely silent (not even logged) and double-clicking is possible. Wrap the await in a try/catch: on entry disable both buttons, on success proceed exactly as today (set status text, hide it -> show it, clear buttons), on failure show a short Arabic error (add and use a `<p class="error-text" id="respond-error" hidden></p>` element near the buttons) and re-enable both buttons so the user can retry. Keep every other behavior on this page exactly as it is. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/offer.html, the photo gallery currently sets the same `alt` text (offer.property_title) on every image when there are multiple photos, which isn't descriptive per-image. Change it so each image's alt text includes its position, e.g. `${offer.property_title} — صورة ${index + 1} من ${total}` (1-indexed), using the photo's index in the array and the total count. Keep every other behavior on this page exactly as it is. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/auth.html, the form's submit handler never disables `#submit-btn` during the async signIn/signUpBuyer call, allowing a double-click double-submit. At the top of the submit handler (right after `e.preventDefault()` and clearing the error), set `submitBtn.disabled = true`; in the `if (error)` branch where the error is shown, also set `submitBtn.disabled = false` so the user can retry (no need to re-enable on the success path, since redirectAfterAuth() navigates away). Keep every other behavior on this page exactly as it is. Touch only site/auth.html — nothing under lib/, no other file.
- [x] Create a new file site/js/properties.js exporting four functions for the public.realtor_properties table (realtor_id column, RLS scopes rows to the signed-in realtor), mirroring the exact style of site/js/offers.js (import supabase from ./supabase-client.js and getCurrentAppUserId from ./auth.js): `listMyProperties()` — selects all columns from realtor_properties where realtor_id = current app user id, ordered by created_at descending, throws on error, returns data; `createProperty(fields)` — inserts `{...fields, realtor_id: await getCurrentAppUserId()}` into realtor_properties, select().single(), throws on error, returns data; `updateProperty(id, fields)` — updates realtor_properties set fields where id = id, select().single(), throws on error, returns data; `deleteProperty(id)` — deletes from realtor_properties where id = id, throws on error. Touch only site/js/properties.js — nothing under lib/, no other file.
- [x] Create a new file site/realtor-properties.html, a page for a realtor to manage their saved property inventory (public.realtor_properties table, columns: title, address, price, currency, lease_type, area_sqft, bedrooms, bathrooms, furnished — no photo_urls form field, leave that column unset for now). Follow the exact structure/style of site/request-new.html (nav bar copied from site/my-offers.html with nav-links "عروضي" -> my-offers.html, "تصفح الطلبات" -> realtor-requests.html, "الإشعارات" -> notifications.html, and a #profile-menu-slot per site/js/auth.js's mountProfileMenu pattern used on every other authenticated page — call `requireSession()` then `mountProfileMenu(document.getElementById("profile-menu-slot"))` at the top of the module script). Page title "عقاراتي المحفوظة". Body: a form with fields for title (text, required), address (text, required), price (number), currency (select: IQD/USD, default IQD), lease_type (select: rent/sale), area_sqft (number), bedrooms (number), bathrooms (number), furnished (checkbox); a submit button calling `createProperty` from ./js/properties.js with the form values, then reloading the list and resetting the form; and below the form, a `<ul id="properties-list">` rendering each saved property from `listMyProperties()` as an `<li class="card">` showing title/address/price/currency, with a "حذف" button per item calling `deleteProperty(id)` then re-rendering the list. Add a `<p id="loading-msg">جاري التحميل...</p>` shown before the initial listMyProperties() call and hidden in a try/finally once it settles, matching the loading-indicator pattern already used on site/my-offers.html. Touch only site/realtor-properties.html — nothing under lib/, no other file.
- [x] In site/realtor-dashboard.html, the nav bar's `.nav-links` list currently does not link to a properties-management page. Add one more item to it: "عقاراتي المحفوظة" linking to realtor-properties.html, in the same `<li><a href="...">...</a></li>` style as its existing items. Change nothing else in the file. Touch only site/realtor-dashboard.html — nothing under lib/, no other file.
- [x] In site/my-offers.html, the nav bar's `.nav-links` list currently has "عروضي", "تصفح الطلبات", "الإشعارات". Add one more item: "عقاراتي المحفوظة" linking to realtor-properties.html, same `<li><a>` style. Change nothing else in the file. Touch only site/my-offers.html — nothing under lib/, no other file.
- [x] In site/realtor-requests.html, the nav bar's `.nav-links` list currently has "الطلبات المتاحة", "عروضي", "الإشعارات". Add one more item: "عقاراتي المحفوظة" linking to realtor-properties.html, same `<li><a>` style. Change nothing else in the file. Touch only site/realtor-requests.html — nothing under lib/, no other file.
- [x] In site/create-offer.html, above the existing offer form's first field, add a `<div class="form-field"><label for="saved-property">اختر من عقاراتي المحفوظة (اختياري)</label><select id="saved-property"><option value="">— إدخال يدوي —</option></select></div>`. In the module script, import `listMyProperties` from ./js/properties.js, call it once after the page's existing initial setup, and populate the select with one `<option>` per saved property (value = property.id, text = property.title). Add a "change" listener on #saved-property: when a non-empty value is chosen, find the matching property from the fetched list and set the existing form fields' values from it (property-title from title, property-address from address, offered-price from price, currency from currency, lease-type from lease_type, area-sqft from area_sqft, bedrooms from bedrooms, bathrooms from bathrooms, furnished checkbox from furnished) — inspect the existing field ids in this file first and match them exactly; do not rename any existing field id. When "— إدخال يدوي —" is reselected, leave the current field values as-is (do not clear them). Do not change the existing submit handler or any other behavior in the file. Touch only site/create-offer.html — nothing under lib/, no other file.
- [BLOCKED] Create a new file site/realtor-profile.html, a public-ish page (behind requireSession() like every other authenticated page — import from ./js/auth.js) showing one realtor's profile: reads a `realtor_id` query param from the URL, calls the existing RPCs `get_realtor_rating_summary(p_realtor_id)` and `list_realtor_reviews(p_realtor_id)` (call them directly via the supabase client imported from ./js/supabase-client.js, e.g. `supabase.rpc("get_realtor_rating_summary", { p_realtor_id: id })`), and renders: the realtor's average rating and total review count from the summary RPC, and a list of individual reviews (rating + comment + created_at) from list_realtor_reviews. Follow the nav bar and #profile-menu-slot pattern from site/offer.html (nav-links: "طلباتي" -> dashboard.html). Add a `<p id="loading-msg">جاري التحميل...</p>` shown before the initial RPC calls and hidden in a try/finally once they settle. If realtor_id is missing from the URL or the RPC calls fail, show a `<p class="error-text" id="page-error">تعذّر تحميل الملف الشخصي</p>` instead of the content. Touch only site/realtor-profile.html — nothing under lib/, no other file.
- [x] In site/offer.html, inside the block that builds the offer-detail card (the one appending h1/pAddress/pPrice/pDesc/pMessage to `card`), after that block, add a link to the realtor's public profile: `<a class="btn" href="realtor-profile.html?realtor_id=${encodeURIComponent(offer.realtor_id)}">ملف الوسيط</a>`, appended to `detail` alongside the existing "الرسائل" link. Change nothing else in the file. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/offer.html, add a WhatsApp contact link for the realtor: in the module script, after `offer` is fetched, use the existing `supabase` client (already imported) to select `whatsapp_phone` from `realtors` where `user_id = offer.realtor_id` (single()); if a non-empty phone is returned, strip any non-digit characters from it and append an `<a class="btn" target="_blank" rel="noopener">تواصل عبر واتساب</a>` to the `detail` element with `href` set to `https://wa.me/` + the digits-only phone. If the query errors or returns no phone, skip adding the link entirely (no error shown to the user for this optional feature). Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/js/auth.js, inside `mountProfileMenu`, after the existing `signoutBtn` is appended to `dropdown`, add a second button `deleteBtn` (type="button", className "btn btn-ghost", textContent "حذف الحساب") appended to `dropdown` right after signoutBtn. Its click handler: show a `window.confirm("هل أنت متأكد من حذف حسابك؟ لا يمكن التراجع عن هذا الإجراء.")`; if confirmed, call `await supabase.auth.getSession()` to get the current access token, then `fetch` a POST request to `` `${supabase.supabaseUrl}/functions/v1/delete-account` `` with headers `Authorization: Bearer <access_token>` and `apikey: <the same anon key supabase-client.js uses — import SUPABASE_ANON_KEY from ./config.js>`; on a non-ok response, `window.alert("تعذّر حذف الحساب، حاول مرة أخرى.")`; on success, call `signOut()` (already defined in this file) to sign the browser out and redirect. Keep every other part of mountProfileMenu exactly as it is. Touch only site/js/auth.js — nothing under lib/, no other file.
- [x] In site/dashboard.html, when the initial listMyRequests() call fails (e.g. an expired/invalid session token — the API call itself fails with an auth error even though a stale session object still exists client-side, so requireSession()'s own check doesn't catch it), the page currently hangs on the loading message forever with no feedback. In the catch branch (or add one if none exists) around the initial listMyRequests() call: hide the loading message, and show a `<p class="error-text" id="load-error">تعذّر تحميل الطلبات — قد تكون جلستك انتهت. <a href="auth.html">سجّل الدخول من جديد</a></p>` element (add it near `#requests-list` if it doesn't exist) instead of leaving the page stuck. Keep every other behavior on this page exactly as it is. Touch only site/dashboard.html — nothing under lib/, no other file.
- [x] In site/auth.html, the applyMode() function's `mode === "reset"` branch currently hides the name field, password field, and forgot-password row, but leaves the Google sign-in button (`#google-btn`) and the "أو سجّل بالبريد الإلكتروني" divider text (`.divider-text`) visible, which don't belong on a password-reset screen. In that same branch, also set `document.getElementById("google-btn").hidden = true` and hide the divider-text element (give it an id if it doesn't have one, e.g. `#divider-text`, or select it via `document.querySelector(".divider-text")`). In the other branches (signin/signup), make sure both are shown again (`hidden = false`). Keep every other behavior on this page exactly as it is. Touch only site/auth.html — nothing under lib/, no other file.
- [x] In site/js/requests.js, `listMyRequests()` currently does `select("*")` on property_requests with no filter, relying entirely on RLS to scope results to the caller. This is wrong for any user whose account is also a verified realtor (every realtor account is a buyer account that got elevated): the realtor-facing RLS policy on this table additionally grants SELECT on any *other* buyer's active request, so a realtor visiting site/dashboard.html ("طلباتي" — my requests) sees strangers' requests mixed into their own list. Fix: add an explicit `.eq("buyer_id", await getCurrentAppUserId())` to the query in `listMyRequests()` so it only ever returns the caller's own requests regardless of what RLS additionally permits. Keep every other function in this file exactly as it is. Touch only site/js/requests.js — nothing under lib/, no other file.
- [x] In site/offer.html, the "مهتم"/"غير مهتم" response buttons have two bugs: (1) on page load, `renderButtons()` is called unconditionally and nothing reflects `offer.buyer_response` if it was already set in a previous visit, so after a page refresh the UI never shows which response (if any) was actually saved; (2) inside `respond(response)`, on success `buttons.innerHTML = ""` permanently removes both buttons with no way to change the response again without a full reload. Fix: right after `renderButtons()` is called on initial load, if `offer.buyer_response` is `"interested"` or `"not_interested"`, set `status.textContent` to the same messages already used in `respond()` ("تم إرسال اهتمامك" / "تم تسجيل عدم الاهتمام") and set `status.hidden = false`, but do NOT clear `buttons.innerHTML` — leave both buttons available so the response can be changed again. Also remove the `buttons.innerHTML = ""` line inside `respond()`'s success path (keep the rest of `respond()`, including the `offer.buyer_response = response` line and the review-section reveal, exactly as they are) so the buttons stay usable after responding, not just on initial load. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/offer.html, inside `renderReviewSection()`, the 5 star rating buttons are appended to `#review-stars` in DOM order value=1,2,3,4,5 (left to right in source order). Because the page is RTL, this visually renders with star value 5 on the left and value 1 on the right, which is backwards from how a 1-to-5 rating should read in RTL (the reader expects the first/rightmost star to be the lowest or the fill to start from the right) — clicking what looks like the 4th star from the right actually sets a different rating than expected. Fix: build and append the star buttons in reverse order (value 5, then 4, then 3, then 2, then 1) so their DOM order matches the intended RTL visual reading order, while keeping each button's `dataset.value` and click behavior (`rating = value; paintStars();`) exactly as they are today. Touch only site/offer.html — nothing under lib/, no other file.
- [x] In site/realtor-profile.html, the page currently only renders a rating-summary heading and a reviews list, with no header identifying which realtor this is. Before the existing `<div class="card"><h1 id="rating-summary">` block, add a small header section: fetch the realtor's `full_name` and `email` from `public.users` (via the existing `supabase` client, `select("full_name, email").eq("id", realtorId).single()` — note `id` here is the public.users.id, matching the `realtor_id` query param already used) and render them as a `<h1 id="realtor-name"></h1>` and `<p id="realtor-email"></p>` above the existing rating card, populated inside the same `load()` function before the rating-summary content is set. If this query fails, leave the name/email elements empty rather than failing the whole page (the rating/reviews content should still render). Touch only site/realtor-profile.html — nothing under lib/, no other file.
- [x] In site/request.html, several tags render raw English/slug values instead of Arabic labels: `purposeTag.textContent = request.purpose` shows "rent"/"buy" raw, `subtypeTag.textContent = request.property_subtype` shows "apartment" raw, and `rentalTag.textContent = request.rental_period` shows "monthly" raw. Also, `govTag` (`govTag.textContent = request.governorate`, a raw slug like "baghdad") is redundant with `cityTag` (which already shows the correct Arabic governorate label, per how site/request-new.html saves both fields) and should be removed entirely rather than translated — delete the `govTag` element and its `tags.append(...)` reference. Fix the remaining three: add inline label maps at the top of the module script, e.g. `const PURPOSE_LABELS = { rent: "إيجار", buy: "شراء" };`, `const SUBTYPE_LABELS = { apartment: "شقة", house: "منزل", villa: "فيلا", duplex: "دوبلكس" };`, `const RENTAL_PERIOD_LABELS = { daily: "يومي", weekly: "أسبوعي", monthly: "شهري", yearly: "سنوي" };`, and use `PURPOSE_LABELS[request.purpose] || request.purpose` (same pattern for the other two) when setting each tag's textContent. Also fix `areaTag.textContent = request.area`: import the default export from `./js/iraq-locations.js` (e.g. `import iraqLocations from "./js/iraq-locations.js"`) and set it to `iraqLocations[request.governorate]?.areas.find(a => a.value === request.area)?.label || request.area` (falls back to the raw value for free-text "أخرى" entries, which are already Arabic). Keep every other tag and all other behavior on this page exactly as it is. Touch only site/request.html — nothing under lib/, no other file.
- [x] In site/realtor-requests.html, `catTag.textContent = r.category` shows the raw English category value ("residential"/"commercial"/"land") instead of an Arabic label, and `budgetTag.textContent = r.max_price != null ? \`${r.max_price}\` : "—"` shows the raw number with no thousands separator or currency suffix. Fix both: add `const CATEGORY_LABELS = { residential: "سكني", commercial: "تجاري", land: "أرض" };` near the top of the module script and use `CATEGORY_LABELS[r.category] || r.category` for catTag (matching the same mapping already used in site/dashboard.html); import `formatThousands` from `./js/numerals.js` and change the budgetTag line to `r.max_price != null ? \`${formatThousands(r.max_price)} ${r.currency ?? "د.ع"}\` : "—"`. Keep every other tag and all other behavior on this page exactly as it is. Touch only site/realtor-requests.html — nothing under lib/, no other file.
- [x] In site/notifications.html, the line `dateTag.textContent = n.created_at ? new Date(n.created_at).toLocaleString("ar") : ...` renders the time before the date (e.g. "6:42:53 ص، 2026/9/17"), which reads unnaturally in Arabic UIs that conventionally show the date first. Change it to build the string explicitly date-first: `` `${new Date(n.created_at).toLocaleDateString("ar-IQ")} ${new Date(n.created_at).toLocaleTimeString("ar-IQ")}` `` (or equivalent single-Date-instance version). Keep every other behavior on this page exactly as it is. Touch only site/notifications.html — nothing under lib/, no other file.
- [x] In site/js/properties.js, `listMyProperties()` currently has no caller-side protection against a network/table error surfacing as an unhandled rejection that can halt whatever page called it before other initialization code (like attaching a form's submit handler) runs. In site/create-offer.html, find where `listMyProperties()` is called during page setup and wrap that call in a try/catch: on failure, `console.error(err)` and leave the "اختر من عقاراتي المحفوظة" select with just its "— إدخال يدوي —" option (i.e., skip populating it), but do not let the failure interrupt any other initialization in this script (the offer form and its submit handler must still work even if the saved-properties fetch fails). Touch only site/create-offer.html — nothing under lib/, no other file.
- [x] Create a new file site/admin-requests.html, an admin page listing every property request on the platform (not just active ones — admin already has full RLS access via the property_requests_admin_all policy, just query `supabase.from("property_requests").select("*, users!property_requests_buyer_id_fkey(full_name, email)").order("created_at", { ascending: false })` directly). Follow the exact nav bar / #profile-menu-slot / requireSession() pattern from site/admin-reports.html (nav-links: "توثيق الوسطاء" -> admin-realtors.html, "البلاغات" -> admin-reports.html, add a third item "كل الطلبات" -> admin-requests.html, self-link). Render each request as an `<li class="card">` showing: title, the joined buyer's full_name and email, category (translate via `{ residential: "سكني", commercial: "تجاري", land: "أرض" }`), status, purpose (`purpose === "rent" ? "إيجار" : "شراء"`), city, and created_at. Add a `<p id="loading-msg">جاري التحميل...</p>` shown before the initial fetch and hidden in a try/finally once it settles, and a `<p class="error-text" id="page-error" hidden></p>` shown on fetch failure, matching the pattern in site/admin-reports.html. Touch only site/admin-requests.html — nothing under lib/, no other file.
- [x] In site/admin-realtors.html, the nav bar's `.nav-links` list currently has "توثيق الوسطاء" (self-link) and "البلاغات". Add one more item after them: "كل الطلبات" linking to admin-requests.html, same `<li><a>` style. Change nothing else in the file. Touch only site/admin-realtors.html — nothing under lib/, no other file.
- [x] In site/admin-reports.html, the nav bar's `.nav-links` list currently has "توثيق الوسطاء" and "البلاغات" (self-link). Add one more item after them: "كل الطلبات" linking to admin-requests.html, same `<li><a>` style. Change nothing else in the file. Touch only site/admin-reports.html — nothing under lib/, no other file.
