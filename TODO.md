...
- [ ] Update the property_requests schema section in CLAUDE.md to document the five new columns added by the migrations above, matching their exact names, types, and constraints. Touch only CLAUDE.md — nothing under lib/, nothing under site/, no SQL file.
- [ ] Create site/js/iraq-locations.js exporting one plain object mapping each of Iraq's 18 governorate slugs to { label: Arabic name, areas: [{ value, label }, ...] }, 8 to 15 common areas per governorate, every list ending with { value: "other", label: "أخرى" }. Touch only this one new file — nothing under lib/, nothing else under site/.
- [ ] Create site/js/numerals.js exporting one function toWesternDigits(str) that maps Arabic-Indic digits ٠-٩ to Western digits 0-9 and leaves every other character unchanged. Touch only this one new file — nothing under lib/, nothing else under site/.
