...
- [ ] In site/my-offers.html, the nav bar's `.nav-links` list currently has "عروضي", "تصفح الطلبات", "الإشعارات". Add one more item: "عقاراتي المحفوظة" linking to realtor-properties.html, same `<li><a>` style. Change nothing else in the file. Touch only site/my-offers.html — nothing under lib/, no other file.
