...
- [ ] In site/offer.html, add a "الرسائل" link/button near the existing offer details, navigating to `messages.html?offer_id=${offerId}`. Change nothing else in the file. Touch only site/offer.html — nothing under lib/, no other file.
- [ ] In site/my-offers.html, add a "الرسائل" link/button to each offer card, navigating to `messages.html?offer_id=${o.id}` (using each offer's id from the existing listMyOffers() data already rendered on this page). Change nothing else in the file. Touch only site/my-offers.html — nothing under lib/, no other file.
