...
- [ ] In site/admin-realtors.html, the nav bar's `.nav-links` list currently has "توثيق الوسطاء" (self-link) and "البلاغات". Add one more item after them: "كل الطلبات" linking to admin-requests.html, same `<li><a>` style. Change nothing else in the file. Touch only site/admin-realtors.html — nothing under lib/, no other file.
