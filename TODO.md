...
- [x] In site/js/auth.js, add an exported async function signUpRealtor(email, password, fullName, companyName, licenseNumber, licenseExpiry) that calls supabase.auth.signUp with options.data set to { role: "realtor", full_name: fullName }, then on success fetches the newly-created row from public.users by auth_id (the new auth user's id) and inserts a matching row into public.realtors with that user's id as user_id, company_name: companyName, license_number: licenseNumber, license_expiry: licenseExpiry. Return the same { data, error } shape the existing signUpBuyer returns, surfacing any insert error from the realtors table the same way. Touch only site/js/auth.js — nothing under lib/, no other file.
